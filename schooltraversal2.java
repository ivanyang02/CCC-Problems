...
public class schooltraversal2 {
	...
	static void build() {
		...
		tbl = new int[n + 1][d + 1];
		cost = new int[n + 1][d + 1];
		for (int i = 1; i <= n; i++) {
			tbl[i][0] = par[i];
			cost[i][0] = graphcost[i][par[i]];
		}
		for (int i = 1; i <= d; i++) {
			for (int j = 1; j <= n; j++) {
				tbl[j][i] = tbl[tbl[j][i - 1]][i - 1];
				cost[j][i] = cost[tbl[j][i - 1]][i - 1];
			}
		}
	}
